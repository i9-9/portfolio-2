...
export default function ProfileLayoutV2() {
  ...
  const [workFilter,    setWorkFilter]    = useState<WorkFilter>("web");
    ...
    <div className="min-h-screen bg-background lg:cursor-none relative">
      ...
      <section id="work" ref={workRef} className="px-4 lg:px-12 py-20">
        ...
        <motion.div
          ...
          className="mb-10 flex flex-wrap items-center gap-x-0 gap-y-2 text-[10px] font-helveticaNowTextRegular uppercase tracking-[0.2em] text-muted-foreground"
        >
          {(["web", "graphic"] as const).map((key, i) => (
            <span key={key} className="inline-flex items-center">
              {i > 0 ? (
                <span className="pointer-events-none mx-3 text-foreground/20" aria-hidden>
                  ·
                </span>
              ) : null}
              <button
              ...
              >
                {key === "graphic" ? t("work.filterGraphic") : t("work.filterWeb")}
              </button>
            </span>
          ))}
        </motion.div>
