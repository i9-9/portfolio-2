#!/usr/bin/env node
/**
 * Record Ursula Benavidez hero slideshow for portfolio previews.
 * Run: node scripts/capture-ursulabenavidez-preview-video.mjs [desktop|mobile|both]
 */
...
const URL = "https://www.ursulabenavidez.com/";

const PRESETS = {
  desktop: {
    viewport: { width: 1440, height: 900 },
    deviceScaleFactor: 2,
    outFile: "ursulabenavidez-preview-desktop.mp4",
    userAgent:
  ...
  mobile: {
    ...
    deviceScaleFactor: 2,
    outFile: "ursulabenavidez-preview-mobile.mp4",
    userAgent:
...
async function waitForHeroSlider(page) {
  await page.waitForSelector("section.absolute.inset-0 img", {
    timeout: 30_000,
  });
  console.log("  hero slider visible");
    ...
    await page.waitForFunction(
      () =>
        new Promise((resolve) => {
          const readAlt = () =>
            document
              .querySelector("section.absolute.inset-0 img")
              ?.getAttribute("alt") ?? "";

          const first = readAlt();
          setTimeout(() => resolve(readAlt() !== first && first.length > 0), 1400);
        }),
      { timeout: 12_000 },
    );
    console.log("  slideshow advancing");
  } catch {
    console.warn("  slideshow did not advance — recording static hero");
  }
...
async function capturePreset(presetKey) {
  ...
  const webmFull = path.join(
    OUT_DIR,
    `ursulabenavidez-preview-${presetKey}-full.webm`,
  );
  fs.renameSync(webmTemp, webmFull);

  const mp4Full = path.join(
    OUT_DIR,
    `ursulabenavidez-preview-${presetKey}-full.mp4`,
  );
  execSync(
...
(async () => {
  ...
  if (!targets) {
    console.error(
      "Usage: node scripts/capture-ursulabenavidez-preview-video.mjs [desktop|mobile|both]",
    );
    process.exit(1);
  }
